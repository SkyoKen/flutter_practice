...
このリポジトリは `Cyber Table Order` という Flutter 練習アプリです。Dart package 名は `cyber_table_order` です。現在のアプリ本体は、サイバーパンク風の飲食店・テーブル注文 UI で、言語切り替え、疑似会員ログイン、QR 注文ダイアログ、メニュー絞り込み、カート管理、注文確定、会計、メモリ上の注文履歴を扱います。
...
- 指示がない限り、現在の見た目を維持してください。暗いサイバーパンク調、deep orange / blue のアクセント、コンパクトな操作部、角丸控えめ、Courier 系のシステム文字が基本です。
